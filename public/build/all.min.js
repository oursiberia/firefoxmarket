...
app.config(function($stateProvider, $urlRouterProvider, $locationProvider, $httpProvider) {
    ...
    navigator.id.watch({
        ...
        function notifyMe(message) {
            if ("Notification" in window) if ("granted" === Notification.permission) {
                new Notification("Firefox Market", {
                    icon: icon,
                    body: message
                });
            } else "denied" !== Notification.permission && Notification.requestPermission(function(permission) {
                if ("permission" in Notification || (Notification.permission = permission), "granted" === permission) {
                    new Notification("Firefox Market", {
                        body: message,
                        icon: icon
                    });
        ...
        var final_manifest = "";
        final_manifest = void 0 !== manifest ? manifest : $scope.manifest, "hosted" === app_type ? (console.log("installing as hosted app"), 
        req = navigator.mozApps.install(final_manifest)) : "packaged" === app_type ? (console.log("installing as packaged app"), 
        req = navigator.mozApps.installPackage(final_manifest)) : "privileged" === app_type ? (console.log("privialied or some other new type"), 
        req = navigator.mozApps.installPackage(final_manifest)) : req = navigator.mozApps.install(final_manifest), 
        req.onsuccess = function() {
            console.log("Install process initiated"), notifyMe(appname + " was installed");
        }, req.onerror = function(e) {
            console.log("Install process failed"), console.log(this.error.name), console.log(e), 
    ...
    var rules = {
        ...
        getImage: function(app) {
            ...
            switch (ratings.body) {
              case "generic":
                for (var types = rules.bodies[0].rating_types, i = 0; i < types.length; ++i) image = types[i].rating === ratings.rating ? types[i].img : rules.general.img;
                break;

              case "esrb":
                for (var rule = rules.bodies[1], a = 0; a < rule.rating_types.length; ++a) image = rule.rating_types[a].ratingNumber === ratings.rating ? rule.rating_types[a].img : rules.general.img;
            }
            return image;
        }
    };
    ...
    var query = "", $scope = null, API = {
        ...
        query: function(_$scope, term) {
            var results = document.getElementsByClassName("resultsbyname")[0], close = document.getElementsByClassName("close-search")[0], homeclose = document.querySelector("#home-search-close");
            if ($scope = _$scope, void 0 === term) {
                if (term = window.document.querySelector("#homesearch") || window.document.querySelector("#search"), 
                "" === term.value) return console.log("no search term entered"), results.style.display = "none", 
                close.style.display = "none", null !== homeclose && (homeclose.style.display = "none"), 
                !1;
                query = term.value, results.style.display = "block", close.style.display = "block", 
                null !== homeclose && (homeclose.style.display = "block");
            }
            $http({
                method: "GET",
                ...
                $scope.author_results = author_results, $scope.author_results_count = author_results.length > 0 ? author_results.length : 0, 
                $scope.name_results = name_results, $scope.name_results_count = name_results.length > 0 ? name_results.length : 0, 
                $scope.category_results = category_results, $scope.category_results_count = category_results.length > 0 ? category_results.length : 0, 
                $scope.description_results = description_results, $scope.description_results_count = description_results.length > 0 ? description_results.length : 0;
            }).error(function(data) {
        ...
        filter: function(appset) {
            for (var len = appset.length, newset = [], i = 0; len - 1 > i; ++i) {
                var app = appset[i], name = $rootScope.filterName(app), icon = app.icons[64], ratings = app.ratings.average, id = app.id;
                newset.push({
                    name: name,
                    icon: icon,
    ...
    return {
        ...
        link: function($scope, $el) {
            $scope.submitAbuse = function() {
                ...
                id = id[id.length - 1];
                var abuse = $el[0].children[0].getElementsByTagName("textarea")[0], request = $http({
                    method: "post",
    ...
    return {
        ...
        link: function($scope, $el, $attr) {
            var appid = $attr.appid, length = $attr.length;
            API.request("ratings", "?app=" + appid).then(function(data) {
                for (var i = 0; i < data.objects.length; ++i) {
                    var review = data.objects[i];
                    review.body = $filter("LongCopyFilter")(review.body, "", length), review.body = $sce.trustAsHtml(review.body), 
                    $scope.copy = review.body, $scope.rating = review.rating, $scope.username = review.user.display_name;
                }
            });
        }
    };
    ...
    return {
        ...
        link: function($scope, $el, $attrs) {
            ...
            id = id[id.length - 1], $scope.name = $attrs.name, $scope.submitReview = function() {
                ...
                rating = parseInt(rating), (rating > 5 || isNaN(rating)) && (rating = 5);
                var data = {
                    app: parseInt(id),
                    body: content.value,
                    rating: rating
    ...
    return {
        ...
        link: function($scope, $el, $attr) {
            function applyStars() {
                for (var i = 0; i < starratings.length; ++i) rating > i && (starratings[i].children[0].className = "starfilled");
            }
    ...
    return {
        ...
        link: function($scope) {
            ...
            $scope.returnHome = function() {
                ...
                var search = document.querySelector("#mainsearch"), inputs = document.getElementsByClassName("homesearch");
                TweenMax.to(search, 1, {
                    height: 0,
                    ease: "Power3.easeInOut",
                    onComplete: function() {
                        for (var i = 0; i < inputs.length; ++i) null !== inputs[i] && (inputs[i].value = "");
                        $rootScope.unlockBody();
                    }
                });
            }, $scope.initPurchase = function(app_type, manifest, appname, icon) {
                console.log(app_type, manifest, appname, icon), $rootScope.initPurchase(app_type, manifest, appname, icon);
            }, $scope.searchApp = function() {
                MasterSearch.query($scope);
            }, $scope.viewSearchResults = function(classname) {
                var search = (document.getElementsByClassName(classname), document.getElementById("search")), metric = "";
                switch (classname) {
                  case "name_results":
                    metric = "Name";
                    break;
                ...
                if (null !== search.value) {
                    ...
                    localStorage.setItem("search-results", data), $location.path("/searchresults/" + search.value);
                }
            };
        }
    };
    ...
    var id = $window.location.href.split("/");
    id = id[id.length - 1], $scope.author = decodeURIComponent(id), API.request("search", "?q=" + decodeURIComponent(id)).then(function(data) {
        for (var objects = data.objects, apps = [], i = 0; i < objects.length; ++i) {
            var oapp = objects[i];
            oapp.author === decodeURIComponent(id) && apps.push({
    ...
    id = id[id.length - 1], $scope.appid = id, API.request("ratings", "?app=" + id + "&limit=999999").then(function(data) {
        ...
        $scope.author = data.author, $scope.urisafe_author = encodeURIComponent(data.author), 
        $scope.categories = data.categories, $scope.category = data.categories[0], $scope.content_ratings = data.content_ratings, 
        $scope.rating = data.ratings.average, $scope.manifest = data.manifest_url, $scope.rating_image = AgeRatingLookup.getImage(data), 
        $scope.name = $rootScope.filterName(data), $scope.version = data.current_version, 
        ...
        API.request("search", "?q=" + data.author).then(function(odata) {
            ...
            if (objects.length > 0 && apps.length < 2) {
                for (var i = 0; i < objects.length; ++i) {
                    var oapp = objects[i];
                    oapp.name[navigator.language] != data.name[navigator.language] && oapp.author === $scope.author && apps.push({
        ...
        target.getAttribute("data-tabname")) {
          ...
          case "screenshots":
            ...
            for (var previews = $scope.previews, a = 0; a < previews.length; ++a) {
                var image = new Image();
                image.src = previews[a].image, image.setAttribute("large", previews[a].image), previewWrap.appendChild(image);
            }
        ...
        for (var featured_reviews = [], all_reviews = [], i = 0; 4 > i; ++i) {
            var review = data.objects[i];
            review.body = $filter("LongCopyFilter")(review.body), featured_reviews.push({
                copy: review.body,
        ...
        $scope.reviews = featured_reviews, $scope.all_reviews = all_reviews;
    });
    var tabs = document.getElementsByClassName("tab-trigger")[0];
    $scope.changeTabContent(tabs, !0), $scope.initPurchase = function(app_type, manifest, appname, icon) {
        $rootScope.initPurchase(app_type, manifest, appname, icon);
        ...
        var box = document.querySelector(modalName), token = localStorage.getItem("token");
        "#review" === modalName ? null === token || void 0 === token ? alert("We're sorry but you need to be logged in in order to write a review") : loadBox() : ("#review" !== modalName || null !== token || void 0 !== token) && loadBox();
        ...
        if ("SPAN" === e.target.tagName) {
            ...
            var recentapps = document.querySelector(".app-reviews"), allreviews = document.querySelector(".review-grid");
            switch (target.className = "active", target.innerHTML) {
              case "Top Reviews":
                recentapps.style.display = "block", allreviews.style.display = "none";
                break;

              case "All Reviews":
                recentapps.style.display = "none", allreviews.style.display = "block";
            }
        }
    };
} ]), app.controller("CategoryViewController", [ "$scope", "API", "$rootScope", "$http", "$filter", function($scope, API, $rootScope, $http, $filter) {
    ...
    function loadCategoryApps() {
        API.request("apps_in_category", category + "&limit=" + searchLimit + "&offset=" + popular_offset).then(function(data) {
            ...
            for (var i = 0; i < objects.length; ++i) {
                var obj = {};
                obj.icon = objects[i].icons[64], obj.name = $rootScope.filterName(objects[i]), obj.id = objects[i].id, 
                obj.author = objects[i].author, obj.classname = "app", obj.app_type = objects[i].app_type || "hosted", 
                obj.manifest_url = objects[i].manifest_url, obj.rating = objects[i].ratings.average, 
                popular_set.push(obj);
            }
            0 >= popular_offset ? ($scope.popularapps = popular_set, console.log("HERE")) : setTimeout(function() {
                $scope.popularapps.concat(popular_set), $scope.$apply();
            }, 100), $rootScope.loaded(), popular_offset += searchLimit;
        });
    }
    function loadRecentApps() {
        API.request("apps_in_category", "&cat=" + category + "&limit=" + searchLimit + "&sort=created&offset=" + recent_offset).then(function(data) {
            ...
            0 >= recent_offset ? $scope.recentapps = recent_set : setTimeout(function() {
                $scope.recentapps.concat(recent_set), $scope.$apply();
            }, 100), recent_offset += searchLimit;
        });
    }
    ...
    var title = document.querySelector("#category-title"), categoryIcon = document.querySelector("#category-icon");
    title.innerHTML = category.charAt(0).toUpperCase() + category.slice(1), categoryIcon.src = "/img/category-icons/" + title.innerHTML + ".svg";
    ...
    API.request("featured", "&cat=" + category).then(function(data) {
        ...
        for (var editorials = data, sections = [], i = 0; i < editorials.length; ++i) {
            var item = editorials[i];
            item.type === category && sections.push(item);
        }
    ...
    var popular_offset = 0, recent_offset = 0, popular_set = [], recent_set = [], searchLimit = 16;
    loadCategoryApps(), loadRecentApps();
    var grid = document.querySelectorAll(".app-grid")[0], timer = (grid.children[0], 
    grid.children[1], setInterval(function() {
    ...
    $scope.toggleCategoryTypes = function(e) {
        ...
        loadRecentApps(), loadCategoryApps();
    };
...
} ]), app.controller("home", [ "$rootScope", "API", "$scope", "$filter", "$http", "MasterSearch", "$location", function($rootScope, API, $scope, $filter, $http, MasterSearch, $location) {
    function buildUrl(app) {
        var chars = app.split("");
        return "https://marketplace.cdn.mozilla.net/img/uploads/addon_icons/" + chars[0] + chars[1] + chars[2] + "/" + app + "-64.png?modified=crushed";
    }
    ...
    $scope.test = {
        ...
        console.log(apps);
        var index = Math.floor(Math.random() * apps.length), featured = apps[index];
        featured.classname = featured.classname + " highlighted";
        for (var first = [], i = 0; 4 > i; ++i) apps[i].description = $filter("LongCopyFilter")(apps[i].description, apps[i].id), 
        first.push(apps[i]);
        ...
        var selector = document.querySelector("#search-home-result"), inputs = document.getElementsByClassName("homesearch"), close = document.getElementsByClassName("close-search");
        TweenMax.to(selector, 1, {
            height: 0,
            onComplete: function() {
                for (var i = 0; i < inputs.length; ++i) null !== inputs[i] && (inputs[i].value = ""), 
                null !== close[i] && (close[i].style.display = "none");
                $rootScope.unlockBody();
            }
        });
    }, $scope.toggleCategoryTypes = function(e) {
        ...
        var search = (document.getElementsByClassName(classname), document.getElementById("homesearch")), metric = "";
        switch (classname) {
          case "name_results":
            metric = "Name";
            break;
        ...
        if ("" !== search.value) {
            ...
            localStorage.setItem("search-term", search.value), localStorage.setItem("search-metric", metric);
            var data = JSON.stringify($scope[classname]);
            localStorage.setItem("search-results", data), $location.path("/searchresults/" + val);
        }
    };
} ]), app.controller("SearchResults", function($scope, $rootScope) {
    var data = JSON.parse(localStorage.getItem("search-results")), term = localStorage.getItem("search-term");
    $scope.search_results = data, $scope.metric = localStorage.getItem("search-metric"), 
    $scope.search_term = term, $rootScope.loaded();
}), app.filter("LongCopyFilter", function() {
    return function(data, appid, length) {
        var charlimit = 160;
        void 0 !== length && (charlimit = length), delimiter = ' <a href="/app/' + appid + '" class="text-delimiter">[...]</a>';
        ...
        return count >= charlimit && (short_review[short_review.length - 1] += delimiter), 
    ...
    return function(data, length) {
        ...
        for (var i = 0; limit > i; ++i) for (var obj = {}, a = 0; a < data[i].device_types.length; ++a) {
            var name = "";
            ...
            void 0 === type && (type = "hosted"), "desktop" === data[i].device_types[a] && (obj.icon = data[i].icons[64], 
            obj.app_type = type, obj.name = $rootScope.filterName(data[i]), obj.id = data[i].id, 
            obj.manifest_url = data[i].manifest_url, obj.premium_type = data[i].premium_type.charAt(0).toUpperCase() + data[i].premium_type.slice(1), 
