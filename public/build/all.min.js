...
app.config(function($stateProvider, $urlRouterProvider, $locationProvider, $httpProvider) {
    ...
    title.innerHTML = category.charAt(0).toUpperCase() + category.slice(1), API.request("featured").then(function() {}), 
    ...
    API.request("featured").then(function(data) {
        var apps = $filter("DesktopApps")(data), index = Math.floor(Math.random() * apps.length), featured = apps[index];
    ...
    return function(data, length) {
        var apps = [], limit = 0;
        limit = length ? length : data.objects.length;
        for (var i = 0; limit > i; ++i) for (var obj = (data.objects[i].name["en-US"], {}), a = 0; a < data.objects[i].device_types.length; ++a) "desktop" === data.objects[i].device_types[a] && (obj.icon = data.objects[i].icons[64], 
        obj.name = data.objects[i].name["en-US"], obj.id = data.objects[i].id, obj.author = data.objects[i].author, 
        obj.classname = "app", obj.device = "desktop", console.log(obj), apps.push(obj));
        return console.log(apps), apps;
    };
});