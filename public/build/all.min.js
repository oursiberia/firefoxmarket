...
app.config(function($stateProvider, $urlRouterProvider, $locationProvider, $httpProvider) {
    ...
    var query = "", $scope = null, API = {
        ...
        query: function(_$scope, term, limit) {
            if (console.log("querying"), $scope = _$scope, void 0 === term) {
            ...
            $http({
                ...
                if (void 0 === limit) $scope.author_results = author_results, $scope.name_results = name_results, 
                $scope.category_results = category_results, $scope.description_results = description_results; else {
                    var author_r = [], name_r = [], category_r = [], description_r = [];
                    if (author_results.length > 4) for (var i = 0; limit > i; ++i) author_r.push(author_results[i]); else author_r = author_results;
                    if (name_results.length > 4) for (var i = 0; limit > i; ++i) name_r.push(name_results[i]); else namer_r = name_results;
                    if (category_results.length > limit) for (var i = 0; limit > i; ++i) category_r.push(category_results[i]); else category_r = category_results;
                    if (description_results.length > limit) for (var i = 0; limit > i; ++i) description_r.push(description_results[i]); else description_r = description_results;
                    $scope.author_results = author_r, $scope.name_results = name_r, $scope.category_results = category_r, 
    ...
    return {
        ...
        controller: function() {},
        link: function($scope, $el, $attrs) {
            console.log($attrs), $scope.name = $attrs.name;
            var id = window.location.href.split("/");
            id = id[id.length - 1], $scope.submitReview = function() {
                var content = $el[0].querySelector("textarea"), rating = $el[0].querySelector("input").value;
                rating = parseInt(rating), (rating > 5 || 0/0 !== rating) && (rating = 5);
                var data = {
                    app: id,
                    body: content.value,
                    rating: rating
    ...
    $scope.changeTabContent(tabs, !0), $scope.initPurchase = function(app_type, manifest) {
        ...
        box.addEventListener("click", function() {
            TweenMax.to(box, .5, {
                opacity: 0,
                onComplete: function() {
                    document.getElementsByTagName("html")[0].style.overflow = "scroll", box.className = "modal closed";
                }
            });
        });
    }, $scope.writeReview = function() {
        if (null === localStorage.getItem("username") && void 0 === localStorage.getItem("username")) return void alert("you must be logged in to write a review");
        var box = document.querySelector("#review");
        box.className = box.className.replace("closed", ""), "0" === box.style.opacity && (document.getElementsByTagName("html")[0].style.overflow = "hidden", 
        TweenMax.to(box, .4, {
            opacity: 1
        ...
        box.className = box.className.replace("closed", ""), document.getElementsByTagName("html")[0].style.overflow = "hidden", 
        "0" === box.style.opacity && (document.getElementsByTagName("html")[0].style.overflow = "hidden", 
        TweenMax.to(box, .4, {
            opacity: 1
        })), box.addEventListener("click", function(e) {
            "modalBox" === e.target.className && TweenMax.to(box, .5, {
        ...
        TweenMax.to(box, .4, {
            ...
            console.log(e.target.tagName), "modalBox" === e.target.className && TweenMax.to(box, .5, {
                opacity: 0,
                onComplete: function() {
                    document.getElementsByTagName("html")[0].style.overflow = "scroll", box.className = "modal closed";
                }
    ...
    $scope.test = {
        ...
        for (var objects = data.objects, popular = [], i = 0; i < objects.length; ++i) {
            var obj = {};
    ...
    return function(data, length) {
        ...
        obj.app_type = data[i].premium_type, obj.name = data[i].name["en-US"], obj.id = data[i].id, 
