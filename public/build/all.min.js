...
}), app.controller("AppViewController", function($window) {
    var id = $window.location.href.split("/");
    id = id[id.length - 1], console.log(id);
}), app.controller("myapps", function() {
    navigator.mozApps.getInstalled();
}), app.controller("home", [ "$rootScope", "API", "$scope", "$window", "$location", function($rootScope, API, $scope) {
    API.request("featured").then(function(data) {
        console.log(data);
        for (var apps = [], i = 0; i < data.objects.length; ++i) {
            var name = data.objects[i].name["en-US"];
            if (void 0 !== name) {
                name = name.split(" "), name = name.join("");
                var obj = {
