...
app.config(function($stateProvider, $urlRouterProvider, $locationProvider, $httpProvider) {
    ...
    function request(endpoint, params, extradata) {
        ...
        return console.log("Need to run : " + endpoint), void 0 !== extradata && (extra = extradata), 
        $timeout(function() {
        ...
        link: function($scope, $el, $attr) {
            ...
            if ("true" === $attr.review) var s = setInterval(function() {
                ...
                if (stars = stars.split("."), 1 == stars.length) rating = stars[0]; else {
                    if (void 0 !== stars[1]) var decimals = stars[1].split("");
    ...
    id = id[id.length - 1], $scope.appid = id, API.request("ratings", "?app=" + id + "&limit=999999").then(function(data) {
        ...
        $scope.category = data.categories[0], $scope.content_ratings = data.content_ratings, 
        $scope.rating = data.content_ratings.rating, $scope.name = data.name[navigator.language], 
        ...
        content.innerHTML = "";
        var target = e.target;
        switch ("H4" === e.target.tagName && (target = e.target.parentNode), target.getAttribute("data-tabname")) {
          case "description":
        ...
        console.log(reviews), $scope.reviews = reviews;
    });
    ...
    function query() {
        ...
        return "" == term.value ? (console.log("no search term entered"), closeSearch(), 
        !1) : void $http({
            method: "GET",
            url: "https://marketplace.firefox.com/api/v1/apps/search/?q=" + term.value + "&device=desktop"
        }).success(function(data) {
    ...
    API.request("featured").then(function(data) {
        ...
        for (var categories = [], i = 0; 4 > i; ++i) {
            var rand = Math.floor(Math.random() * data.objects.length), cat = data.objects[rand];
            categories.push(cat);
        }
    ...
    return function(data) {
        for (var charlimit = 90, review = data.split(" "), short_review = [], i = 0; i < review.length; ++i) {
            var word = review[i], count = 0, len = word.split("").length;
            count += len, charlimit > count && short_review.push(word);
        }
        return short_review = short_review.join(" ") + " ...";
    };
