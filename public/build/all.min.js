...
app.config(function($stateProvider, $urlRouterProvider, $locationProvider, $httpProvider) {
    ...
    var query = "", $scope = null, API = {
        ...
        filter: function(appset) {
            for (var len = appset.length, newset = [], i = 0; len - 1 > i; ++i) {
                var app = appset[i], name = app.name[navigator.language], icon = app.icons[64], ratings = app.ratings.average;
                newset.push({
                    name: name,
                    icon: icon,
                    ratings: ratings
...
}), app.directive("abusedirective", [ "API", function() {
    return {
    ...
    $scope.changeTabContent(tabs, !0), $scope.initPurchase = function(app_type) {
        if (console.log("clicked"), -1 === navigator.userAgent.search("Firefox")) return void alert("We're sorry, but apps within the Firefox Marketplace can only be downloaded from the Firefox browser");
        switch (app_type) {
          case "hosted":
        ...
        box.className = box.className.replace("closed", ""), document.getElementsByTagName("html")[0].style.overflow = "hidden";
    }, $scope.writeReview = function() {
        if ("true" !== localStorage.getItem("loggedIn")) return void alert("you must be logged in to write a review");
    ...
    function swapApp(first) {
        ...
        first ? (app = fapps[0], currentIndex = 0, TweenMax.to(selector, 1.5, {
            ...
            onComplete: function() {
                ...
                app = fapps[currentIndex], console.log(app);
                var img = new Image();
                img.src = app.icon;
                var h1 = document.createElement("h1"), h3 = document.createElement("h3"), div = document.createElement("div");
                h1.innerHTML = app.name, h3.innerHTML = app.author, selector.appendChild(img), selector.appendChild(h1), 
                selector.appendChild(h3), selector.appendChild(div), div.setAttribute("data-starrating", ""), 
                div.setAttribute("data-rating", app.rating.average), TweenMax.to(selector, 1, {
                    opacity: 1
                });
            }
        });
