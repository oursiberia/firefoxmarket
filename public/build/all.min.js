...
app.config(function($stateProvider, $urlRouterProvider, $locationProvider, $httpProvider) {
    ...
    id = id[id.length - 1], API.clientRequest("app_detail", id).then(function(data) {
    ...
    category = category[category.length - 1], API.clientRequest("apps_in_category", category).then(function(data) {
    ...
    API.clientRequest("featured").then(function(data) {
        var apps = $filter("DesktopApps")(data, 10), index = Math.floor(Math.random() * apps.length), featured = apps[index];
        featured.classname = featured.classname + " highlighted";
        for (var i = 0; i < apps.length; ++i) if (i === index) {
            var temp = apps[0];
            apps[0] = apps[index], apps[index] = temp;
        }
        $scope.featured_apps = apps, $http({
            method: "GET",
